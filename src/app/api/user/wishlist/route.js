...
export async function POST(req) {
  try {
    ...
    if (existing) {
      ...
      return new Response(
        JSON.stringify({
          message: "Wishlist status toggled",
          status: !existing.status,
        }),
        { status: 200 }
      );
      ...
      return new Response(
        JSON.stringify({ message: "Wishlist created", status: true }),
        { status: 200 }
      );
    }
  } catch (err) {
...
export async function GET(req) {
  try {
    ...
    if (book_id) {
      ...
      return new Response(
        JSON.stringify({ status: wishlist?.status || false }),
        { status: 200 }
      );
    }
    ...
    const { data: books, error: booksError } = await supabase
      .from("book")
      .select(
        "book_id, title, author, publisher, cover, price_standard, price_sales, stock, sales_count, status"
      )
      .in("book_id", bookIds)
      .eq("status", true);
