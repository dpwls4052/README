...
export async function POST(req) {
  try {
    ...
    if (!userId) return NextResponse.json({ errorMessage: "userId 필요" }, { status: 400 });
    ...
    if (!user) return NextResponse.json({ errorMessage: "존재하지 않는 회원입니다." }, { status: 404 });
    ...
    console.error(err);
    return NextResponse.json({ errorMessage: err.message || "서버 에러" }, { status: 500 });
  }
}
