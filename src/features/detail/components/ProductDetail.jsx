...
const MOCK_DETAIL_TABS_DATA = {
    description: `이 책은 독자들에게 깊은 감동과 인사이트를 제공하는 훌륭한 작품입니다. 
      ...
      독서의 즐거움을 만끽할 수 있습니다.`,
    reviews: [
      { id: 1, author: "독서광123", rating: 5, date: "2025-01-15", content: "정말 감동적인 책이었습니다. 강력 추천합니다!", avatar: "https://bit.ly/dan-abramov" },
      { id: 2, author: "책벌레", rating: 4, date: "2025-01-10", content: "내용이 알차고 좋았어요. 다만 중반부가 조금 지루했습니다.", avatar: "https://bit.ly/kent-c-dodds" },
      { id: 3, author: "리더777", rating: 5, date: "2025-01-05", content: "인생 책으로 등극! 여러 번 읽고 싶네요.", avatar: "https://bit.ly/ryan-florence" },
    ],
    faqs: [
      { id: 1, question: "배송은 얼마나 걸리나요?", answer: "일반적으로 주문 후 2-3일 내에 배송됩니다." },
      { id: 2, question: "반품/교환이 가능한가요?", answer: "상품 수령 후 7일 이내 미개봉 상태에 한해 반품/교환이 가능합니다." },
    ],
};
...
const DEFAULT_BOOK_DATA = {
    ...
    description: "전직 FBI요원이자 행동전문가인 조 내버로가 상대방의 몸짓과 표정을 읽음으로써 사람의 마음을 간파해 효과적인 커뮤니케이션을 할 수 있는 기술을 담은 책이다. ...", // 실제 긴 내용은 DetailPage 내에서 MOCK_DETAIL_TABS_DATA.description 대신 사용됨.
    id: "9788901110806",
...
const DetailPage = () => {
    ...
    // 로딩 상태 처리
    if (dataLoading && !bookFromDB) { // 데이터 로딩 중이고, 기존 데이터도 없을 때만 로딩 표시
        return (
    ...
    if (!detailData.id) {
        return (
            <Container maxW="1200px" py="100px">
                <Text fontSize="xl">상품 데이터를 불러올 수 없거나 존재하지 않습니다. (ID: {idx})</Text>
    ...
    const averageRating = totalReviews > 0
        ? (detailData.reviews.reduce((sum, review) => sum + review.rating, 0) / totalReviews).toFixed(1)
        : 0;
        ...
        <Container maxW="1200px" p="0" margin="100px auto">
            ...
            <Box borderTop="2px solid var(--main-color)" pt="40px">
                ...
                {activeTab === "description" && (
                    <Box py="40px">
                        <Text fontSize="var(--font-medium)" lineHeight="1.8" whiteSpace="pre-line">
                        ...
                        {detailData.faqs?.length > 0 ? (
                            detailData.faqs.map((faq) => (
                                ...
                                >
                                    ...
                                    <Text fontSize="var(--font-medium)" color="gray.700" pl="16px">
                                        A. {faq.answer}
                                    </Text>
                                </Box>
                            ))
